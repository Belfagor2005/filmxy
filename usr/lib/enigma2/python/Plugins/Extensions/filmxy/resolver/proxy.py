...
def request(url, check):
    try:
        print "Here in proxy-py url =", url
        print "Here in proxy-py check =", check
        result = client.request(url)
#        print "Here in proxy-py url, check, result =", url, check, result
        if check in str(result):
            return result.decode('iso-8859-1').encode('utf-8')
#        print "Here in proxy-py get() =", get()
        result = client.request(get() + urllib.quote_plus(url))
#        print "Here in proxy-py url, check, result 2=", url, check, result
        if check in str(result):
            return result.decode('iso-8859-1').encode('utf-8')
#        print "Here in proxy-py get() 2=", get()
        result = client.request(get() + urllib.quote_plus(url))
#        print "Here in proxy-py url, check, result 3=", url, check, result
        if check in str(result):
            return result.decode('iso-8859-1').encode('utf-8')
    except:
        return
...
def get():
    return "http://www.pingproxy.com/browse.php?b=20&u="


def getX():
    return random.choice(['http://2anonymousproxy.com//browse.php?b=20&u=',

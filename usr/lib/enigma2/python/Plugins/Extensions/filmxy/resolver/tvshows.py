...
class tvshows:
    ...
    def get(self, url, idx=True):
        try:
            pass  # pass#print "In tvshows-py url =", url
            try: url = getattr(self, url + '_link')
            except: pass
            pass  # pass#print "In tvshows-py url 2=", url
            try: u = urlparse.urlparse(url).netloc.lower()
            except: pass
            pass  # pass#print "In tvshows-py u =", u

            if u in self.trakt_link and '/users/' in url:
                pass  # pass#print "In tvshows-py 1"
                try:
                ...
                pass  # pass#print "In tvshows-py 2"
                self.list = cache.get(self.trakt_list, 0, url, self.trakt_user)

            elif u in self.trakt_link:
                pass  # pass#print "In tvshows-py 3"
                self.list = cache.get(self.trakt_list, 24, url, self.trakt_user)
                if idx == True: self.worker()


            elif u in self.imdb_link and ('/user/' in url or '/list/' in url):
                pass  # pass#print "In tvshows-py 4"
                self.list = cache.get(self.imdb_list, 0, url)
                self.worker()

            elif u in self.imdb_link:
                pass  # pass#print "In tvshows-py 5"
                self.list = cache.get(self.imdb_list, 24, url)
                self.worker()


            elif u in self.tvmaze_link:
                pass  # pass#print "In tvshows-py 6"
                self.list = cache.get(self.tvmaze_list, 168, url)
                self.worker()


            if idx == True: self.tvshowDirectory(self.list)
            pass  # pass#print "In tvshows-py self.list =", self.list
            return self.list
        except:
            pass
    ...
    def imdb_list(self, url):
        pass  # pass  # print "In tvshows imdb_list url =", url
        try:
            dupes = []

            for i in re.findall('date\[(\d+)\]', url):
                url = url.replace('date[%s]' % i, (self.datetime - datetime.timedelta(days = int(i))).strftime('%Y-%m-%d'))
            pass  # pass  # print "In tvshows imdb_list url 2=", url
            def imdb_watchlist_id(url):
                return re.findall('/export[?]list_id=(ls\d*)', client.request(url))[0]
            pass  # pass  # print "In tvshows imdb_list url 3=", url
            """
            if url == self.imdbwatchlist_link:
                url = cache.get(imdb_watchlist_id, 8640, url)
                url = self.imdblist_link % url
                pass  # pass#print "In tvshows imdb_list url 4=", url
            elif url == self.imdbwatchlist2_link:
                url = cache.get(imdb_watchlist_id, 8640, url)
                url = self.imdblist2_link % url
            """
            pass  # pass#print "In tvshows imdb_list url 5=", url
            result = client.request(url)
            pass  # pass#print "In tvshows imdb_list result =", result
            result = result.replace('\n','')
            result = result.decode('iso-8859-1').encode('utf-8')
            pass  # pass#print "In tvshows imdb_list result 2=", result
            items = client.parseDOM(result, 'div', attrs = {'class': 'lister-item mode-advanced'})
            items += client.parseDOM(result, 'div', attrs = {'class': 'list_item.+?'})
            pass  # pass#print "In tvshows imdb_list items =", items
        except:
            return
        ...
        for item in items:
            pass  # pass#print "In tvshows imdb_list item =", item
            try:
    ...
    def super_info(self, i):
        try:
            ...
            if imdb == '0':
                url = self.imdb_by_query % (quote_plus(self.list[i]['title']), self.list[i]['year'])
